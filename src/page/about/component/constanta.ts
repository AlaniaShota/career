export const title = "About Us";
export const description =
  "We connect talent with top employers across the globe.";
export const secondaryDescription =
  "SJob is a premier platform that helps job seekers find positions with leading companies worldwide. We are committed to making the job search process simple, fast, and effective.";
export const mission = "Our Mission";
export const help =
  "We aim to help everyone find the right job and build a successful career by providing access to thousands of job listings and useful resources. Our goal is to empower professionals with the tools they need to grow, connect, and thrive.";

export const lorem =
  "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Libero, error. ";
