...
export default function JobCards({ jobs, selectedJobId, onSelect }: Props) {
  ...
  const filteredJobs = useMemo(() => filterAndSortJobs(jobs, filters), [
    jobs,
    filters,
  ]);
    ...
    <motion.div layout className="flex flex-col p-4 bg-white rounded-2xl">
      <div className="flex justify-between items-start">
        <div className="flex items-center gap-4">
          <motion.div variants={cardInnerItem} className="p-2 rounded-2xl shadow-md bg-white">
            <img src={job.company.logo} alt={job.company.name} className="w-16 h-16" />
          </motion.div>
